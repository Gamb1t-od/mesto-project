...
const openPopupPic = function(popup){
  {
    popup.classList.add('popup_opened_pic');
  }
}
...
const closePopupPic = function(popup){
  {
  popup.classList.remove('popup_opened_pic');
  }
}
...
function AddCard(evt){
  evt.preventDefault();
  const element = createElement(jobInputCard.value, nameInputCard.value)
...
elements.addEventListener('click',function(evt)
{
  ...
  if (target.id === 'element__image'){
    ...
    openPopupPic(popupPicOpen)();
  }
})

buttonClosePic.addEventListener('click', () => closePopupPic(popupPicOpen));
